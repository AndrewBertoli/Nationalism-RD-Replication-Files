...
# You can also get the functions BalancePlot(), PermutationTest(), and RDPlot() from my GitHub page.
 

data=read.xls("WorldCupData.xls",sheet=1,header=TRUE)
...
tAGG=apply(t[,11:30], 2, sum)
cAGG=apply(c[,11:30], 2, sum)
...
ggplot(tsdata, aes(x=Year)) + geom_line(aes(y=tAGG), colour="Blue", show_guide=TRUE) + geom_line(aes(y=cAGG) , colour="gray44", show_guide=TRUE) + geom_hline(yintercept=0) + geom_vline(xintercept=-0.5, linetype=2) + geom_vline(xintercept=0, linetype=2) + annotate("text", x=-0.68, y=12.2, angle=90, label="Qualification", size=4.2) + scale_x_continuous(breaks = c(-5.5,-4.5,-3.5,-2.5,-1.5,1,2,3,4), labels = c(-5,-4,-3,-2,-1,1,2,3,4)) + annotate("text", x=-0.2, y=12.2, angle=90, label="World Cup", size=4.2) + labs(title="Figure 2. Comparing Aggression Before and After the World Cup") + ylab("Militarized Interstate Disputes Initiated") + xlab("Years Since Qualification and the World Cup") + theme_bw() + geom_rect(aes(xmin = -5.4, xmax = -3.1, ymin = 13.37, ymax = 14.9),fill="white",colour="black") + annotate("text", x = c(-5.15, -5.15, -4.4, -4.1), y = c(14.88, 14.2, 14.5, 13.8), label = c("_", "_", "Qualifiers", "Non-qualifiers"), colour=c("Blue", "gray54", "black", "black"), size=c(8, 8, 4, 4)) + theme(plot.title = element_text(lineheight=.8, face="bold"))
dev.off()




# Post-treatment Statistical Tests
...
# Excluding US-Soviet Union Pairs

t.test(with(t[t$GreatPower==0,], AGGAfter-AGGBefore), with(c[t$GreatPower==0,], AGGAfter-AGGBefore),paired=TRUE)
...
# Countries that Qualified or Missed By 3 points or Less
...
PermutationTest(t$AGGAfter-t$AGGBefore, c$AGGAfter-c$AGGBefore, Paired=TRUE)
...
wilcox.test(t$AGGAfter,c$AGGAfter,paired=TRUE)
...
ggplot(teffects[-7,], aes(position, treatment.effects)) + geom_point()  + theme_bw() + scale_x_continuous(breaks = position, labels = names) + xlab("Variable") + ylab("Increase in Probability of Dispute\nInitiation (from Baseline)") + scale_y_continuous(breaks = seq(0,100,by=20), labels = c("0%","20%","40%","60%","80%","100%"),limits=c(0,105))+geom_hline(yintercept=0)+ labs(title="Figure 4. Comparing the Effect of the World\nCup to Other Estimated Treatment Effects")+ theme(plot.title = element_text(lineheight=.8, face="bold",size=18), axis.text.x  = element_text(angle=90, vjust=0.5, hjust=1, size=12), axis.title= element_text(size=16))+geom_point(data=subset(teffects,treatment.effects==33.3),colour="red")
...
# Change in Military Participation and Military Expenditures
...
t.test(MilperPercentAfter-Milper/Tpop~Treat,sample,paired=TRUE)
...
BalancePlot(Data= sample[sample$GreatPower==0,], Treat= sample[sample$GreatPower==0, ]$Treat, Title="Figure 1a. Balance for the World Cup Sample Excluding the US and Soviet Union", Title.Size=0.95, Covariates= c("Irst", 'Milex','Milper', 'Tpop','Upop', 'BirthRate','DeathRate', 'InfantMortality','Energy','Imports','Exports','LandArea','CINC','Democracy','GreatPower','EngagedCivilWar','EndedCivilWar','EntranceYear','SexRatio','LifeExpectancy','MedianAge','Alliances','USAlly','SoccerMostPopular','PrevAppear','AGGYearBefore','AGG3YearsBefore','AGG5YearsBefore'), Names.To.Print=c('Iron and Steel Production','Military Expenditures', 'Military Personnel', 'Total Population', 'Urban Population', 'Birth Rate', 'Death Rate', 'Infant Mortality', 'Energy Production', 'Imports', 'Exports', 'Land Area', 'Material Power Score', 'Level of Democracy', 'Great Power Status', 'Engaged in Civil War', 'Resolved Civil War', 'Year of State Formation', 'Sex Ratio', 'Life Expectancy', 'Median Age', "Number of Alliances", "U.S. Ally", "Soccer Most Popular Sport", 'Appearance at Previous World Cup',  'MIDs Initiated in the Year Before', 'MIDs Initiated in the 3 Years Before', 'MIDs Initiated in the 5 Years Before'), Shade.Color="cadetblue2", Built.In.Tests= c("T.Test"), Point.Color="black", Year.Covariates= c("EntranceYear"), Paired=FALSE)
dev.off()
...
BalancePlot(Data= sample[abs(sample$PointsFromCutpoint)<=1, ], Treat= sample[abs(sample$PointsFromCutpoint)<=1, ]$Treat, Title="Figure 2a. Balance Plot for the World Cup Sample (1-Point Window)", Title.Size=1, Covariates= c("Irst",'Milex','Milper','Tpop','Upop','BirthRate','DeathRate','InfantMortality','Energy','Imports','Exports','LandArea','CINC','Democracy','GreatPower','EngagedCivilWar','EndedCivilWar','EntranceYear','SexRatio','LifeExpectancy','MedianAge','Alliances','USAlly','SoccerMostPopular','PrevAppear','AGGYearBefore','AGG3YearsBefore','AGG5YearsBefore'), Names.To.Print=c('Iron and Steel Production','Military Expenditures', 'Military Personnel', 'Total Population', 'Urban Population', 'Birth Rate', 'Death Rate', 'Infant Mortality', 'Energy Production', 'Imports', 'Exports', 'Land Area', 'Material Power Score', 'Level of Democracy', 'Great Power Status', 'Engaged in Civil War', 'Resolved Civil War', 'Year of State Formation', 'Sex Ratio', 'Life Expectancy', 'Median Age', "Number of Alliances", "U.S. Ally", "Soccer Most Popular Sport", 'Appearance at Previous World Cup',  'MIDs Initiated in the Year Before', 'MIDs Initiated in the 3 Years Before', 'MIDs Initiated in the 5 Years Before'), Shade.Color="cadetblue2", Built.In.Tests= c("T.Test"), Point.Color="black", Year.Covariates= c("EntranceYear"), Paired=TRUE)
dev.off()
...
ns[i]=dim(newt)[1]}
pdf("SUTVA.pdf")
hist(estimatevector,xlim=c(-1.5,1.5),xlab="Estimate",ylab="Frequency",main="Figure 4a. Estimates for Samples with No Repeated States")
abline(v=mean(t$AGGAfter-t$AGGBefore-(c$AGGAfter-c$AGGBefore)),col="lightblue")
abline(v=mean(estimatevector),col="blue")
legend(-1.5,150,c("Average of Estimates","Estimate from Full Sample"),lwd=c(2.5,2.5),col=c("blue","lightblue"))
dev.off()
...
# Rankings at the Tournament

pdf("WCRankings.pdf", height=4.5)
ggplot(data=t,aes(x=(t$Rank-1)/(t$NumberOfParticipants-1)))+xlab("Rank")+ylab("Frequency")+labs(title="Figure 5a. Rankings of the Qualifiers at the World Cup")+geom_density(adjust=0.25,fill="cornflowerblue") + scale_x_continuous(limits=c(0,1),breaks = c(0.0,1), labels = c("First Place","Last Place"))
dev.off()
