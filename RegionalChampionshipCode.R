...
rcdata=read.csv("https://raw.githubusercontent.com/AndrewBertoli/Nationalism-RD-Replication-Files/master/RegionalChampionshipData.csv")
...
ggplot(tsdata, aes(x=Year)) + geom_line(aes(y=tAGG), colour="Blue", show_guide=TRUE) + geom_line(aes(y=cAGG) , colour="gray74", show_guide=TRUE) + geom_hline(yintercept=0) + geom_vline(xintercept=0, linetype=2) + annotate("text", x=-0.18, y=11, angle=90, label="Qualification", size=3.5) + geom_vline(xintercept=1.142466, linetype=2) + annotate("text", x=0.962466, y=11.6, angle=90, label="Last Tournament", size=3.5) + scale_x_continuous(breaks = c(-3.75,-3,-2.25,-1.5,-0.75,1.142466+3/4,1.142466+3/2,1.142466+9/4), labels = c(-5,-4,-3,-2,-1,1,2,3)) + labs(title="Figure 10a. Comparing Aggression for the Regional Championships") + ylab("Militarized Interstate Disputes Initiated") + xlab("Years Since Qualification and the Last Tournament") + theme_bw() + geom_rect(aes(xmin = -3.9, xmax = -2, ymin = 11.17, ymax = 12.4),fill="white",colour="black")+ annotate("text", x = c(-3.65, -3.65, -3, -2.78), y = c(12.35, 11.829, 12.05, 11.55), label = c("_", "_", "Qualifiers", "Non-qualifiers"), colour=c("Blue", "gray74", "black", "black"), size=c(8, 8, 4, 4))+ theme(plot.title = element_text(lineheight=.8, face="bold",size=14.7),axis.title=element_text(size=13.7),axis.text=element_text(size=11.7))
dev.off()
...
op=par(mfrow=c(1,3), mar=c(4,5,3,0),oma=c(0,0.5,2,0))

bw=rdbwselect(sample$AGG3YearsBefore,sample$PointsFromCutpoint,bwselect="IK")[[3]][1]

RDPlot(X=sample$PointsFromCutpoint,Y=sample$AGG3YearsBefore,xlim=c(-3.7,3.7),ylim=c(-1.5,1.5), Main="Aggression Before", xlab="Points Above/Below Cut-Point",ylab="Militarized Interstate Disputes Initiated",Bandwidth=bw, Window=c(-2.5,2.5), NBoots=10000 , Breaks=c(-11.5,-10.5,-9.5,-8.5,-7.5,-6.5,-5.5,-4.5,-3.5,-2.5,-1.5,-0.5,0,0.5,1.5,2.5,3.5,4.5,5.5,6.5,7.5,8.5,9.5,10.5,11.5,12.5),cex.main=1.8,cex.lab=1.5)

bw=rdbwselect(sample$AGG3YearsAfter,sample$PointsFromCutpoint,bwselect="IK")[[3]][1]

RDPlot(X=sample$PointsFromCutpoint,Y=sample$AGG3YearsAfter,xlim=c(-3.7,3.7),ylim=c(-1.5,1.5), Main="Agression After",xlab="Points Above/Below Cut-Point",ylab="",Bandwidth=bw, Window=c(-2.5,2.5) , NBoots=10000 , Breaks=c(-11.5,-10.5,-9.5,-8.5,-7.5,-6.5,-5.5,-4.5,-3.5,-2.5,-1.5,-0.5,0,0.5,1.5,2.5,3.5,4.5,5.5,6.5,7.5,8.5,9.5,10.5,11.5,12.5),cex.main=1.8,cex.lab=1.5,yaxt='n')

bw=rdbwselect(sample$AGG3YearsAfter-sample$AGG3YearsBefore,sample$PointsFromCutpoint,bwselect="IK")[[3]][1]

RDPlot(X=sample$PointsFromCutpoint,Y=sample$AGG3YearsAfter-sample$AGG3YearsBefore,xlim=c(-3.7,3.7),ylim=c(-1.5,1.5), Main="Change in Aggression", xlab="Points Above/Below Cut-Point",ylab="", Bandwidth=bw, Window=c(-2.5,2.5), NBoots=10000, Breaks=c(-11.5,-10.5,-9.5,-8.5,-7.5,-6.5,-5.5,-4.5,-3.5,-2.5,-1.5,-0.5,0,0.5,1.5,2.5,3.5,4.5,5.5,6.5,7.5,8.5,9.5,10.5,11.5,12.5),cex.main=1.8,cex.lab=1.5,yaxt='n')

title("Figure 11a. Change in Aggression for the Regional Championships",outer=TRUE,cex.main=2)
...
rcdata.use=rcdata.use[,-c(2:3,13:28)]

rcdata.use=rcdata.use[,-c(10,36)]

data=read.csv("WorldCupData.csv")

data.use=data[which(data$Use==1),]

data.use=data.use[,colnames(rcdata.use)]

combined=rbind(data.use,rcdata.use)

pdf("BalancePlotCombined.pdf", width = 7, height = 8)
BalancePlot(Data=combined, Treat=combined$Treat, Title="Figure 12a. Balance Plot for Combined Sample"
...
t1=data[data$Treat==1,c("Country","Year","Score","PointsFromCutpoint","AGGBefore","AGGAfter")]
t2=rcdata[rcdata$Use%in%c(0,1)&rcdata$Treat==1,c("Country","Year","Score","PointsFromCutpoint","AGG3YearsBefore","AGG3YearsAfter")]

colnames(t2)=colnames(t1)

qualifiers=rbind(t1,t2)

c1=data[data$Treat==0,c("Country","Year","Score","PointsFromCutpoint","AGGBefore","AGGAfter")]
c2=rcdata[rcdata$Use%in%c(0,1)&rcdata$Treat==0,c("Country","Year","Score","PointsFromCutpoint","AGG3YearsBefore","AGG3YearsAfter")]
...
bw=rdbwselect(sample$AGGBefore,sample$PointsFromCutpoint,bwselect="IK")[[3]][1]

RDPlot(X=sample$PointsFromCutpoint,Y=sample$AGGBefore,xlim=c(-3.7,3.7),ylim=c(-1.5,1.5), Shade.Color="lightyellow", Main="Aggression Before", xlab="Points Above/Below Cut-Point",ylab="Militarized Interstate Disputes Initiated",Bandwidth=bw, Window=c(-2.5,2.5), Breaks=c(-11.5,-10.5,-9.5,-8.5,-7.5,-6.5,-5.5,-4.5,-3.5,-2.5,-1.5,-0.5,0,0.5,1.5,2.5,3.5,4.5,5.5, 6.5,7.5,8.5,9.5, 10.5,11.5,12.5),NBoots=10000)

bw=rdbwselect(sample$AGGAfter,sample$PointsFromCutpoint,bwselect="IK")[[3]][1]

RDPlot(X=sample$PointsFromCutpoint,Y=sample$AGGAfter,xlim=c(-3.7,3.7),ylim=c(-1.5,1.5), Shade.Color="lightyellow", Main="Aggression After",xlab="Points Above/Below Cut-Point",ylab="",Bandwidth=bw, Window=c(-2.5,2.5), Breaks=c(-11.5,-10.5,-9.5,-8.5,-7.5,-6.5,-5.5,-4.5,-3.5,-2.5,-1.5,-0.5,0,0.5,1.5,2.5, 3.5,4.5,5.5,6.5, 7.5,8.5, 9.5,10.5,11.5,12.5),NBoots=100000)

bw=rdbwselect(sample$AGGAfter-sample$AGGBefore,sample$PointsFromCutpoint,bwselect="IK")[[3]][1]

RDPlot(X=sample$PointsFromCutpoint,Y=sample$AGGAfter-sample$AGGBefore,xlim=c(-3.7,3.7),ylim=c(-1.5,1.5), Shade.Color="blanchedalmond", Main="Change in Aggression", xlab="Points Above/Below Cut-Point",ylab="", Bandwidth=bw, Window=c(-2.5,2.5) , Breaks=c(-11.5,-10.5,-9.5,-8.5,-7.5,-6.5,-5.5,-4.5,-3.5,-2.5,-1.5,-0.5,0,0.5,1.5,2.5,3.5,4.5,5.5,6.5,7.5,8.5,9.5, 10.5,11.5,12.5),NBoots=10000)

title("Figure 13a. Change in Aggression for the Combined Sample",outer=TRUE,cex.main=2)
...
op=par(mfrow=c(1,3), mar=c(4,4.5,3,0),oma=c(0,0,2,0))

bw=rdbwselect(sample$AGGBefore,sample$PointsFromCutpoint,bwselect="IK")[[3]][1]

RDPlot(X=sample$PointsFromCutpoint,Y=sample$AGGBefore,xlim=c(-3.7,3.7),ylim=c(-1.5,1.5), Shade.Color="blanchedalmond", Main="Aggression Before", xlab="Points Above/Below Cut-Point",ylab="Militarized Interstate Disputes Initiated",Bandwidth=bw, Window=c(-2.5,2.5), Breaks=c(-11.5,-10.5,-9.5,-8.5,-7.5,-6.5,-5.5,-4.5,-3.5,-2.5,-1.5,-0.5,0,0.5,1.5,2.5,3.5,4.5,5.5, 6.5,7.5,8.5,9.5, 10.5,11.5,12.5),NBoots=10000,cex.main=1.8,cex.lab=1.5)

bw=rdbwselect(sample$AGGAfter,sample$PointsFromCutpoint,bwselect="IK")[[3]][1]

RDPlot(X=sample$PointsFromCutpoint,Y=sample$AGGAfter,xlim=c(-3.7,3.7),ylim=c(-1.5,1.5), Shade.Color="blanchedalmond", Main="Aggression After",xlab="Points Above/Below Cut-Point",ylab="",Bandwidth=bw, Window=c(-2.5,2.5), Breaks=c(-11.5,-10.5,-9.5,-8.5,-7.5,-6.5,-5.5,-4.5,-3.5,-2.5,-1.5,-0.5,0,0.5,1.5,2.5, 3.5,4.5,5.5,6.5, 7.5,8.5, 9.5,10.5,11.5,12.5),NBoots=100000,cex.main=1.8,cex.lab=1.5,yaxt='n')

bw=rdbwselect(sample$AGGAfter-sample$AGGBefore,sample$PointsFromCutpoint,bwselect="IK")[[3]][1]

RDPlot(X=sample$PointsFromCutpoint,Y=sample$AGGAfter-sample$AGGBefore,xlim=c(-3.7,3.7),ylim=c(-1.5,1.5), Shade.Color="blanchedalmond", Main="Change in Aggression", xlab="Points Above/Below Cut-Point",ylab="", Bandwidth=bw, Window=c(-2.5,2.5) , Breaks=c(-11.5,-10.5,-9.5,-8.5,-7.5,-6.5,-5.5,-4.5,-3.5,-2.5,-1.5,-0.5,0,0.5,1.5,2.5,3.5,4.5,5.5,6.5,7.5,8.5,9.5, 10.5,11.5,12.5),NBoots=10000,cex.main=1.8,cex.lab=1.5,yaxt='n')
